...
AddEventHandler('vRP:hasEnteredMarker', function (zone)
  if zone == 'StartJob' then
	...
	CurrentActionMsg  = ('Press ~INPUT_CONTEXT~ to make a package')
    CurrentActionData = {}
  elseif zone == 'LeaveBox' then
    CurrentAction	  = 'leave_box'
	CurrentActionMsg  = ('Press ~INPUT_CONTEXT~ to hand in the package.')
	CurrentActionData = {}
  ...
  if CurrentAction ~= nil then
    ...
    if IsControlPressed(0,  Keys['E']) then
		...
		CurrentAction = nil
			...
			text = ('Security Guard  »  You do not have any package.'),
			type = "error",
			timeout = (3500),
			layout = "bottomCenter",
			queue = "loffe",
...
AddEventHandler('vRP:startJob', function()
	...
	repeat
	TriggerEvent("vRP:missiontext", '~w~Collecting the package. Time Left: ~r~' .. TimeLeft .. ' ~w~Seconds', 1000)
	TimeLeft = TimeLeft - 1
	Citizen.Wait(1000)
	until(TimeLeft == 0)
